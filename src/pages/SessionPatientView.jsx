...
export default function SessionPatientView() {
      ...
      try {
        ...
        const v =
            c.visitHistory?.find((vh) => {
              ...
              return (typeof appt === "string" ? appt : appt?._id) === appointmentId
            }) || null
        setVisit(v)
  ...
  if (error || !client)
    return (
        <div className="min-h-screen flex flex-col bg-white">
          <Navbar />
          <div className="flex-1 flex items-center justify-center text-red-600">Error loading session.</div>
        </div>
    )
  if (!visit)
    return (
  ...
  const insights = visit.aiInsights?.length ? visit.aiInsights : [PLACEHOLDER_INSIGHT]
  const activities = visit.activities?.length ? visit.activities : [PLACEHOLDER_ACTIVITY]
  ...
  const generateActivity = async () => {
    ...
    try {
      setBusy(true)
      const { data } = await api.post(`/appointments/${appointmentId}/generate-activity`, {
      ...
      <div className="min-h-screen flex flex-col bg-white">
        ...
        <main className="flex-1 p-6 max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-semibold text-gray-800">Session — {client.name}</h2>
          </div>
          ...
          <div className="grid grid-cols-12 gap-6">
            ...
            <div className="col-span-5 bg-[#F9F9FD] rounded-2xl p-6 flex flex-col">
              ...
              <div className="bg-white rounded-xl p-4 mt-6 shadow-sm">
                ...
                <p className="text-sm text-gray-700 mt-1">{(client.pastHistory || []).join(", ") || "—"}</p>
              </div>
              ...
              <div className="bg-white rounded-xl p-4 mt-6 flex-1 shadow-sm">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {(client.goals || []).length > 0 ? (
                      client.goals.map((g) => (
                          <span key={g} className="px-3 py-1 bg-primary/10 text-primary rounded-full text-xs">
                      {g}
                    </span>
                      ))
                  ) : (
              ...
              <div className="bg-white rounded-xl p-4 mt-6 shadow-sm">
                ...
                <div className="text-sm mt-1 text-gray-700 space-y-1">
                  {(visit.note || PLACEHOLDER_VISIT_NOTE).split("\n").map((line, i) => (
            ...
            <div className="col-span-7 flex flex-col space-y-6">
              ...
              <div className="flex gap-4 bg-[#F5F4FB] rounded-2xl px-6 py-4 shadow-sm">
                <TabBtn
                    ...
                    label="Visit Notes"
                />
                <TabBtn id="insights" active={activeTab} setActive={setActiveTab} Icon={Zap} label="Insights" />
                <TabBtn
                  ...
                  <div className="bg-[#F5F4FB] rounded-2xl p-6 shadow-sm space-y-4">
                    ...
                    {insights.map((ins, i) => (
                        <div key={i} className="flex items-center justify-between bg-white rounded-lg p-3">
                          ...
                          <span className={`${ins.tagColor} text-xs font-medium px-2 py-1 rounded-full`}>{ins.tag}</span>
                        </div>
                    ))}
                  </div>
              )}
              ...
              {activeTab === "activityGenerator" && (
                  <div className="bg-[#F5F4FB] rounded-2xl p-6 shadow-sm space-y-6">
                    ...
                    <MultiSelectDropdown
                        ...
                        options={(client.goals || []).map((g) => ({ id: g, label: g }))}
  ...
  return (
      <button
          ...
          className={`flex items-center gap-2 px-9 py-3 text-base font-medium rounded-xl transition border ${
              isActive ? "bg-primary text-white border-primary" : "bg-white text-primary border-gray-200 hover:bg-[#F5F4FB]"
          }`}
      >
