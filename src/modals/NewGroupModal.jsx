...
/* ------------------------------------------------------- */
/*                NEW-GROUP-MODAL (updated with SearchDropDown) */
/* ------------------------------------------------------- */
export default function NewGroupModal({ open, onClose, onCreated }) {
  ...
  const showError = (msg) => {
    setErr({ show: true, msg })
  }

  const validate = () => {
    if (!f.members.length) {
      showError("Pick at least one member")
      return false
    }
    if (!f.date || !f.startTime || !f.endTime) {
      showError("Date / time missing")
      return false
    }
    const start = new Date(`${f.date} ${f.startTime}`)
    const end = new Date(`${f.date} ${f.endTime}`)
    if (isBefore(start, new Date())) {
      showError("Start time is in the past")
      return false
    }
              ...
              <form onSubmit={submit}>
                <div className="grid grid-cols-2 gap-8">
                  ...
                  <div className="space-y-6">
                    ...
                    {/* Members - Updated to use SearchDropDown */}
                    <SearchDropDown
                        label="Members"
                        required={true}
                  ...
                  {/* Right Column - keeping the rest unchanged */}
                  <div className="space-y-6">
                    {/* First Date */}
                    <CustomDatePicker
                        label="First Date *"
