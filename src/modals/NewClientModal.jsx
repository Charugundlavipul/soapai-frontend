...
import { ChevronDownIcon, XMarkIcon, PlusIcon } from "@heroicons/react/24/solid"
...
export default function NewClientModal({ open, onClose, onCreated }) {
  ...
  const addGoal = (g) => !form.goals.includes(g.name) && setForm((f) => ({ ...f, goals: [...f.goals, g.name] }))
        ...
        <Modal open={open} onClose={onClose} title="Create New Client">
          <form onSubmit={submit} className="space-y-6">
            ...
            <div ref={ddRef} className="space-y-1">
              ...
              >
                <span className="text-gray-500">{ddOpen ? "Select goal…" : "+ Click to select goal"}</span>
                  ...
                  <ul className="absolute z-50 mt-1 w-full max-h-56 overflow-y-auto bg-white border rounded-md shadow-lg text-sm">
                    {goalBank.length > 0 ? (
                        ...
                        <li className="px-4 py-2 text-gray-400 text-center">
                          No goals available. Click "Manage Goals" to create some.
                        </li>
                    )}
                  </ul>
              )}
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {form.goals.map((g) => (
                    <span key={g} className="flex items-center gap-1 bg-primary text-white rounded-full px-3 py-1 text-xs">
                  {g}
                      <XMarkIcon onClick={() => removeGoal(g)} className="h-3 w-3 cursor-pointer text-white/80" />
                </span>
                ))}
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button type="button" onClick={onClose} className="px-4 py-2 rounded-xl border border-primary text-primary">
                Cancel
              </button>
